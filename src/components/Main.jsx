...
import {
  ...
  thunder,
  cloud,
} from "../assets/img/icons/weatherIcons";
//icons
...
import { createUser } from "../redux/slices/global/globalSlice";
...
export default function Main({ dataKey, setBgPhrase }) {
  ...
  useEffect(() => {
    dispatch(createUser("Voyager"));
  }, []);

  const unit = useSelector((state) => state.global.unit);
  const myUser = useSelector((state) => state.global.email);
  ...
  const handleChangeClass = (condition, isDay) => {
    ...
    switch (true) {
      ...
      case condition === "Sunny":
        ...
        break;
      default:
        console.log(`Sorry, we are out of conditions`);
        break;
    }
    setClName(newClName);
  };
  ...
  return (
    <Box p="0 10px">
      ...
      >
        <h6>Welcome, {myUser} 👋</h6>
        <h4>Current weather 🌤️</h4>
        <span className="time">
          {time} <FaRegClock />
        </span>
        ...
        >
          ...
          />
          <span className="temp">
            {unit === "C" ? forecastData?.temp_c : forecastData?.temp_f}°
          </span>
        ...
        >
          {additionalData.map((item) => (
            <Box key={item.title} width={{ xs: "calc(50% - 5px)", md: "auto" }}>
