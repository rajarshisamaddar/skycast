...
export default function ForecastCard({ data, selected, setSelected, i }) {
  ...
  return (
    <Box
      className={`${selected === i && "selected"} forecastCard`}
      ...
      <Stack direction="row" justifyContent="space-between">
        ...
        <Box className="show">
          ...
          <Box display="flex" alignItems="center" gap="5px">
            <img
              src={data.hour[3].condition.icon}
              alt={data.hour[3].condition.text}
            />
            <span>
              {unit === "C" ? data.hour[3].temp_c : data.hour[3].temp_f} °
            </span>
          </Box>
        </Box>

        {/* Hide when it is not selected */}
