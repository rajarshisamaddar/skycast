...
export default function HourlyForecast({ dataKey }) {
  ...
  useEffect(() => {
    ...
    const fetchDataCityWeather = async (q) => {
      await axios
        .request(
          { ...weatherHourlyOptions, params: { q: q, days: days } },
        ...
        .then(function (response) {
          ...
          if (days === 1) {
            ...
            const firstArr = response.data.forecast.forecastday[0].hour.slice(
              hour,
              24
            );
            const secondArr = response.data.forecast.forecastday[1].hour.slice(
              0,
              hour
            );
      ...
      >
        ...
        <Stack ref={scroll} className="horizontal-scroll-bar">
          {hourForecast?.map((item, index) => (
            <HourlyCard key={index} data={item} />
          ))}
        </Stack>
      </Box>
    </Box>
